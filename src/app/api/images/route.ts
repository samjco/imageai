...
const outputDir = '/app/generated-images';
    ...
    if (mode === 'generate') {
      ...
      for (const [key, value] of formData.entries()) {
        if (key.startsWith('image_') && value instanceof Blob) {
          // Convert Blob to File with a default name if not provided
          const fileName = value instanceof File ? value.name : `image-${key}.png`;
          const fileType = value instanceof File ? value.type : 'image/png';
          imageFiles.push(new File([value], fileName, { type: fileType }));
        }
      }
      ...
      const maskFile = formData.get('mask') as Blob | null;
      let maskFileObj: File | undefined;

      if (maskFile) {
        const maskFileName = maskFile instanceof File ? maskFile.name : 'mask.png';
        const maskFileType = maskFile instanceof File ? maskFile.type : 'image/png';
        maskFileObj = new File([maskFile], maskFileName, { type: maskFileType });
      }
      ...
      if (maskFileObj) {
        params.mask = maskFileObj;
      }

      console.log('Calling OpenAI edit with params:', {
          ...params,
          image: `[${imageFiles.length} files]`, 
          mask: maskFile ? 'mask file present' : 'N/A'
      });
